  ...
  <div class="post"> 
    ...
    <?php endif; ?>
                    ...
                    <div class="post-actions"> 
                        <form id="like-form" action="include/processa_like.php" method="post" style="display: inline;"> 
                            <input type="hidden" name="desafio_id" value="<?= $desafio_id ?>"> 
                            <button type="submit"><?= $jaCurtiu ? "❤" : "💔" ?></button> 
                        </form> 
               ...
               $sqlSolucoes = "SELECT solucoes.id, solucoes.usuario_id, solucoes.descricao, solucoes.enviado_em, usuarios.nome FROM solucoes JOIN usuarios ON solucoes.usuario_id = usuarios.id WHERE solucoes.desafio_id = ? ORDER BY solucoes.enviado_em ASC";
                ...
                $solucoes = $stmt->get_result();
                 ...
                 <div style="margin-left: 20px; border-left: 2px solid #ccc; padding-left: 10px;">
                  <p class="user-conteudo"><strong><?= htmlspecialchars($solucao['nome']) ?>:</strong> <?= nl2br(htmlspecialchars($solucao['descricao'])) ?></p>
                  <i><small><?= date("d/m/Y H:i", strtotime($solucao['enviado_em'])) ?></small></i>
                ...
                <form action="include/apagar_solucao.php" method="post" style="display:inline;" onsubmit="return confirm('Tem certeza que deseja apagar este comentário?');">
                    <input type="hidden" name="id" value="<?= $solucao['id'] ?>">
                    <button class="editar-apagar" type="submit">Deletar</button>
                </form>
                <br><br>
            <?php endif; ?>
        </div>
...
document.querySelectorAll('#like-form').forEach(form => {
    form.addEventListener('submit', function(e) {
        ...
        fetch('include/processa_like.php', {
            method: 'POST',
            body: formData
        }).then(response => response.text())
          .then(data => {
              // Ex: atualizar número de curtidas ou mudar ícone
              console.log("Curtido!");
              location.reload(); // (ou atualize manualmente o trecho afetado)
          });
    });
});
</script>
